...
void Settings::WriteWiFiConfigToPreferences(char* settings, int size) {

  prefs.begin("wifiprefs", false); // use "wifiprefs" namespace
  uint16_t checksum = CRC16::CalculateArray((uint8_t*)settings, size);
  prefs.putBytes("wificonfig", settings, size);
  prefs.putUInt("wifiCRC", checksum);       //Save checksum
  //Serial.print("Prefs size = "); Serial.print(size);Serial.print("  CRC= ");Serial.println(checksum);
  prefs.end();
}
...
bool Settings::ReadWiFiConfigFromPreferences(char* settings, int size) {
  prefs.begin("wifiprefs");
  prefs.getBytes("wificonfig", settings, size);  //Read prefs
  uint16_t checksum = CRC16::CalculateArray((uint8_t*)settings, size);
  uint16_t existingChecksum = prefs.getUInt("wifiCRC",0);
  prefs.end();
  //for(int i=0; i <16 ; i++) Serial.print(settings[i], HEX);
  if (checksum == existingChecksum) {
    return true;
